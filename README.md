...
Radna verzija aplikacije koja pokriva funkcije potpisnika i ovlašćenih lica skupština može da se vidi/proba [ovde](https://test-einicijativa.one/#/). Više detalja o stanju implementacije planovima možete naći na dnu ove stranice.
...
##### Poslednji put ažurirano: 12.10.2023
### Urađeno
U ovom trenutku su  baza i nipub servis manje-više kompletni (do na defekte, koji se čiste kako budu uočeni), a web sajt pokazuje podatke generisane od strane nipub servisa i implementira dinamičke funkcije koje se tiču akcija potpisnika i ovlašćenih lica skupština. Servis niapi i dinamički deo sajta su trenutno u implementaciji. 
...
### Planovi
Sada će funkcije za inicijatore biti dodate u niapi, a onda i aplikaciju (njih je mnogo manje nego onih za inicijatore). Posle ovoga će biti implementirane funkcije za podšku šalterima, a onda i one koje se okidaju na bazi vremena. Posle toga će biti urađen pregled i ojačavanje "bezbednosti u dubinu" dodatnim smanjivanjem osetljivosti baze na slučaj kada napadač ovlada jednom ili više mašina srednjeg sloja (niapi - pošto je nipub servis potpuno izolovan u read-only modu i proizvodi samo javne podatke).
